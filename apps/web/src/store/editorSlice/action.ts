...
export const modifyloadingAciton = (status: boolean) => {
  console.log('%c=modifyloadingFn==>', 'color:grey',)
  store.dispatch(editorSlice.actions.modifyloading(status));
}
...
export const newVimModeChangeAction = (mode: VimModeChangeArgs) => {
  console.log('%c=newVimModeChangeAction==>A', 'color:grey',)
